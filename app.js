...
// محرك التنزيل المباشر لملفات الـ PDF بدون فتح نافذة الطباعة
...
async function requirePDFEngine() {
    ...
    return new Promise((resolve, reject) => {
        ...
        script.onload = () => { isPdfLoaded = true; resolve(); };
        script.onerror = reject;
        document.head.appendChild(script);
    });
}
...
window.savePDF = async function(title, content, customFileName) {
    ...
    // إنشاء حاوي موقت لالتقاط التقرير
    const tempContainer = document.createElement('div');
    tempContainer.style.cssText = 'position: absolute; left: -9999px; top: -9999px; width: 750px; background: white;';
    tempContainer.innerHTML = getPrintTemplate(title, content);
